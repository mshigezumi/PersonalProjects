...
namespace TbsFramework.AW
{
    public class GuiController : MonoBehaviour
    {
        ...
        private void OnUnitHighlighted(object sender, EventArgs e)
        {
            ...
            var unit = sender as AWUnit;
            UnitNameText.text = unit.UnitName;
            HitPointsText.text = string.Format("HP: {0}/{1}", unit.HitPoints, unit.TotalHitPoints);

            string defenceText = "";
            if ((unit.Cell as AWSquare).DefenceBoost != 0)
            {
                defenceText = string.Format("Def: {0} + {1}", unit.DefenceFactor, (unit.Cell as AWSquare).DefenceBoost);
            }
            else
            {
            ...
            if (unit is AWWizard)
            {
                RangedAttackText.text = string.Format("Ranged: {0}", (unit as AWWizard).RangedAttackFactor);
            }
            else
            {
